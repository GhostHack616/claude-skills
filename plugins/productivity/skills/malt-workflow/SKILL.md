...
1. **Récupère l'offre dans Gmail.** Cherche les mails de Malt (expéditeur `noreply@malt.com`, objet "Nouvelle opportunité avec ..."). Ne retenir que les **notifications d'opportunité / nouvelle mission** : écarter les autres mails Malt (factures, paiements, messages de la plateforme, relances admin, newsletters). Par défaut la plus récente.
2. **Extraire le texte depuis le HTML, à chaque fois.** Le mail Malt est en HTML : le champ `plaintextBody` est souvent vide ou inutilisable, le vrai contenu est dans `htmlBody`. Toujours prendre le `htmlBody`, retirer les balises, déséchapper les entités (`&#39;`, `&euro;`...) et nettoyer les espaces pour obtenir l'offre propre. (Le thread complet est volumineux : si `get_thread` dépasse la limite, extraire le `htmlBody` via un script jq/python sur le fichier sauvegardé plutôt que de tout lire.)
3. **Annoncer en une ligne l'offre retenue** (entreprise + intitulé + date) avant de dérouler. Ne pas bloquer en attente : il déclenche vite, enchaîner sauf objection.
4. Lis l'offre : titre, projet, livrables attendus, profil recherché, secteur. **Extraire aussi les blocs structurés du bas du mail** : `Compétences obligatoires`, `Compétences préférées`, nombre de jours/semaine, dates de début, télétravail, durée, lieu, tarif journalier. Les **compétences obligatoires + préférées sont les mots-clés exacts que Malt a utilisés pour matcher** : ce sont les premiers suspects du diagnostic de ranking (Étape 3B). Ne jamais les sauter.
